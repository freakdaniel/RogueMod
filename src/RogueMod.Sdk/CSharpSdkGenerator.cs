...
public sealed class CSharpSdkGenerator
{
    ...
    public CSharpSdkGenerationResult Generate(UnrealSdkModel model, string outputDirectory, string rootNamespace)
    {
        ...
        return new CSharpSdkGenerationResult(sourcePath, manifestPath, model.Types.Count);
    }
        ...
        IReadOnlySet<string> supportedStructPaths)
    {
        ...
        builder.Append("public class ").Append(typeName).Append(" : ").Append(baseName).AppendLine();
        builder.AppendLine("{");
        ...
        builder.AppendLine("        var handle = unreal.FindFirstOf(UnrealName);");
        builder.Append("        return handle.IsNull ? null : new ").Append(typeName).AppendLine("(unreal, handle);");
        builder.AppendLine("    }");

        var usedMembers = new HashSet<string>(StringComparer.Ordinal)
        {
            typeName, "UnrealPath", "UnrealName", "FindFirst", "Handle", "Unreal", "IsValid", "PathName"
        };
        foreach (var property in type.Properties.Where(property => !HasFlag(property.Flags, "CPF_Parm")))
        {
            builder.AppendLine();
...
public sealed record CSharpSdkGenerationResult(string SourcePath, string ManifestPath, int TypeCount);
