...
internal static class CSharpTypeTranslator
{
        ...
        IReadOnlySet<string> supportedStructPaths)
    {
        CsType result = type.Kind switch
        {
            ...
            "SetProperty" when type.Inner is not null =>
                Container("IReadOnlySet", Resolve(type.Inner, 1, typeNames, supportedStructPaths)),
            "MapProperty" when type.Key is not null && type.Value is not null =>
                new CsType($"IReadOnlyDictionary<{Resolve(type.Key, 1, typeNames, supportedStructPaths).Name}, {Resolve(type.Value, 1, typeNames, supportedStructPaths).Name}>", false, string.Empty),
    ...
    internal static string? ValueDescriptorExpressionOrNull(CsType type, string descriptorOwnerExpression) =>
        type.ArrayAdapter || type.OptionalAdapter ? ValueDescriptorExpression(type, descriptorOwnerExpression) : null;
    ...
    internal static void AppendValueDescriptorInitializer(
        ...
        IReadOnlySet<string> supportedStructPaths)
    {
        if (type.Kind != "OptionalProperty" || type.Inner is null || !IsSupportedOptionalValue(type.Inner, supportedStructPaths))
        {
            return;
        }
