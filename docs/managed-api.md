...
Generated post hooks also set `SkipInputDecoding` because their callback shape contains only the owner, return value, and out/ref values. Pure input parameters are therefore not decoded after the original call. This both avoids unnecessary transport and permits an observational post hook when an unrelated input type is not yet representable by the generated SDK. Low-level hooks retain complete post-call input snapshots by default; callers may opt into the same behavior with `UnrealHookOptions(SkipInputDecoding: true)`.

Script structs are transported field by field through generated adapters. This includes non-`STRUCT_IsPlainOldData` structs with `FString`/`FName`/`FText` fields, whose values are constructed and destroyed through the live struct property rather than copied as raw bytes. The runtime validates the struct path, declared size, field bounds, scalar widths, bool masks, and nested descriptors before crossing the native boundary, and it never copies CLR object layout into an Unreal buffer.
...
Structs with inheritance, container fields, object-reference fields, unsupported fields, or fixed arrays remain `UnrealValue` and fail explicitly when used. Nested containers inside `TOptional`, weak/lazy/soft-object arrays, `FText` arrays with localization-history preservation, and UTF-8/ANSI string properties are not yet marshalled. `TMap` and `TSet` values are readable (scalar keys, nested-array values/elements) and exposed as `IReadOnlyDictionary<K,V>` and `IReadOnlySet<T>`; writes are supported and gated on `UnrealReflectionCapabilities.MapSetWrites`, and the bridge rejects them before touching game memory if the game build's container layout no longer matches. Standalone soft references are generated as `UnrealSoftObjectReference<T>`; use `FromPath` to assign an unloaded asset path without resolving it. Nested `TArray` transport exists, but the current Deadzone JMAP has no live property with that shape, so it is not yet game-confirmed.
